...
<!--            </el-form-item>-->
    ...
    <el-dialog v-bind:title="dataForEdit.id ? '编辑': '添加'" :visible.sync="editDialogShow" width="60%">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" icon="iconfont tes-icon-cancel" @click="cancelCommit">取 消</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      download() {
        ...
        // window.location.href = "http://localhost:8080/judge/api/downloadModel";
        this.downloadFile("/judge/api/downloadModel");
      },
      ...
      * */
      ...
      toDisable(idx, row){
        this.$confirm("禁用后考试不再使用该试题，是否继续?", "提示", {
          ...
          this.doPost("/judge/api/disable?id=" + row.id).then(res => {
            if(res){
              this.$message({
                type: 'success',
                message: '试题禁用成功!'
