...
//              {min: 3, max: 10, message: '长度在3-10个字符', trigger: 'blur'}
    ...
    methods: {
      ...
      async findByCondition() {
        console.log("开始分页查询")
        let data = await this.doPost("/questionPolicy/api/findByCondition?pageNum=" + this.queryForm.pageNum + "&pageSize=" + this.queryForm.pageSize, this.queryForm);
      ...
      updateData: async function () {
        ...
        this.$refs['editForm'].validate(async (valid) => {
          if (!valid) {
            ...
            var res = await this.doPost('/questionPolicy/api/update', this.dataForEdit);
            if (res) {
      ...
      addData: async function () {
        ...
        this.$refs['addForm'].validate(async (valid) => {
          if (!valid) {
            ...
            let res = await this.doPost("/questionPolicy/api/save", this.dataForAdd, 1000);
            if (res == true) {
...
//        关闭对话框
      ...
      async toRemove(idx, row) {
        ...
        let data = await this.doPost("/questionPolicy/api/delete?id=" + row.id);
        if (data == true) {
    ...
    created: async function () {
      console.log("created....")
      this.findByCondition();
      let questionTypeInfo = await this.doPost("/questionType/api/getQuestionTypeInfo");
