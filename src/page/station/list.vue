...
//            ],
    ...
    methods: {
      ...
      findByCondition: async function () {
        let data = await this.http("/station/api/findByCondition?pageNum=" + this.queryForm.pageNum + "&pageSize=" + this.queryForm.pageSize, this.queryForm);
...
//        关闭对话框
      ...
      async toRemove(idx, row) {
        ...
        let data = await this.http("/station/api/delete?id=" + row.id);
        if (data == true) {
